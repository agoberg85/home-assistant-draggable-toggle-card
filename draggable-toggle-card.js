...
class DraggableToggleCard extends LitElement {
  ...
  constructor() {
    ...
    // Initialize properties for cached elements
    this._thumbEl = null;
    this._trackEl = null;
  }

  // NEW: Lifecycle method to prevent memory leaks.
  // This is called automatically when the card is removed from the dashboard.
  disconnectedCallback() {
    super.disconnectedCallback();
    // If the card is removed while a drag is active, clean up the listeners.
    if (this._dragging) {
      this._handleDragEnd();
    }
  }
  ...
  _handleTap(e) {
    ...
    if (isLock) {
      const serviceToCall = currentState === 'locked' ? 'unlock' : 'lock';
      this._interimState = currentState === 'locked' ? 'unlocked' : 'locked';
      this.hass.callService('lock', serviceToCall, { entity_id: this.config.entity });
    } else {
  ...
  _handleDragStart(e) {
    this._thumbEl = this.shadowRoot.querySelector('.thumb');
    this._trackEl = this.shadowRoot.querySelector('.track');

    if (!this._thumbEl || !this._trackEl) return;
    ...
    this._initialPos = { x: this._thumbEl.offsetLeft, y: this._thumbEl.offsetTop };
    this._dragging = true;
    window.addEventListener('mousemove', this._handleDragMove);
    window.addEventListener('touchmove', this._handleDragMove);
  ...
  _handleDragMove = (e) => {
    ...
    const diff = { x: currentPos.x - this._startPos.x, y: currentPos.y - this._startPos.y };
    
    if (this.config.orientation === 'vertical') {
      const newTop = this._initialPos.y + diff.y;
      const maxTop = this._trackEl.offsetHeight - this._thumbEl.offsetHeight;
      this._thumbEl.style.top = `${Math.max(0, Math.min(newTop, maxTop))}px`;
    } else {
      const newLeft = this._initialPos.x + diff.x;
      const maxLeft = this._trackEl.offsetWidth - this._thumbEl.offsetWidth;
      this._thumbEl.style.left = `${Math.max(0, Math.min(newLeft, maxLeft))}px`;
    }
  }
  ...
  _handleDragEnd = (e) => {
    ...
    let dragEndedOn;
    if (this.config.orientation === 'vertical') {
      const center = this._trackEl.offsetHeight / 2;
      dragEndedOn = this._thumbEl.offsetTop < center;
    } else {
      const center = this._trackEl.offsetWidth / 2;
      dragEndedOn = this._thumbEl.offsetLeft < center;
    }
    ...
    const isLock = this.config.entity.startsWith('lock.');
    const currentState = this.hass.states[this.config.entity].state;
    const currentStateIsOn = isLock ? currentState === 'unlocked' : currentState === 'on';
    if (dragEndedOn !== currentStateIsOn) {
      if (isLock) {
        const serviceToCall = dragEndedOn ? 'unlock' : 'lock';
        this._interimState = dragEndedOn ? 'unlocked' : 'locked';
        this.hass.callService('lock', serviceToCall, { entity_id: this.config.entity });
      } else {
        this._interimState = dragEndedOn ? 'on' : 'off';
        this.hass.callService('homeassistant', 'toggle', { entity_id: this.config.entity });
      }
    }
  ...
  static get styles() {
    return css`
      ...
      .horizontal .thumb.off {
        left: var(--offset-off-h);
        top: var(--offset-on);
      }
